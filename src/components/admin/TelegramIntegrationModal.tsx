  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden my-8 max-h-[90vh] flex flex-col">
        ...
        {/* Header */}
        <div className="px-6 py-5 bg-[#2563EB] text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-100">Receive real-time gift card validation & upload alerts directly in Telegram</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-700 dark:text-slate-300">
          ...
          <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${isConfigured ? 'bg-[#86A98D]/20 text-[#86A98D] border border-[#86A98D]/30' : 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30'}`}>
          ...
          <div className="space-y-3">
            ...
            <ol className="list-decimal list-inside space-y-2 text-xs text-slate-600 dark:text-slate-400 pl-1 leading-relaxed">
              <li>Open Telegram and search for <strong className="text-[#2563EB] dark:text-blue-400">@BotFather</strong>.</li>
              <li>Send <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[#2563EB] font-mono">/newbot</code> to create your bot and copy your <strong>Bot API Token</strong>.</li>
              <li>Add your new bot to your Telegram group or channel, or message it directly.</li>
              <li>Get your Chat ID (or send a message to <strong className="text-[#2563EB] dark:text-blue-400">@userinfobot</strong> to see your numeric Chat ID).</li>
              <li>Add <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[#2563EB] font-mono">TELEGRAM_BOT_TOKEN</code> and <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-[#2563EB] font-mono">TELEGRAM_CHAT_ID</code> to your <strong>Render Environment Variables</strong>.</li>
            </ol>
          </div>
          ...
          <form onSubmit={handleTestNotification} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg text-xs bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 focus:ring-2 focus:ring-[#2563EB] dark:text-white font-mono"
                />
              </div>
            ...
            {feedback && (
              <div className={`p-3 rounded-lg text-xs flex items-center gap-2 ${feedback.type === 'success' ? 'bg-[#86A98D]/20 text-[#86A98D] border border-[#86A98D]/30' : 'bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/30'}`}>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold text-xs px-4 py-2 rounded-lg shadow-md flex items-center gap-2 cursor-pointer"
              >
